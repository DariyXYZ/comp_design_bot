...
namespace CompDesignBot.Features.Auth;
...
/// <summary>
/// Вход в Mini App: проверка данных запуска Telegram, свои токены сессии и коды
/// входа из кнопки бота. Все три — подписанный HMAC-SHA256 payload без состояния
/// на сервере, секрет — токен бота. Формат общий с прежними Python/TS-версиями:
/// <c>base64url(payload).base64url(signature)</c>, поэтому выданные раньше
/// токены и кнопки в чатах продолжают работать.
/// </summary>
public sealed class SessionTokens
{
    ...
    /// <summary>Код входа живёт столько же, сколько кнопка в чате — неделями.</summary>
    public const int LoginCodeTtlSeconds = 30 * 24 * 3600;
    ...
    public SessionTokens(string botToken, TimeProvider? clock = null)
    {
        _secret = Encoding.UTF8.GetBytes(botToken);
    ...
    /// <summary>
    /// Код входа, который бот кладёт в адрес кнопки Mini App. Клиенты Telegram
    /// отдают initData непредсказуемо; кнопку формирует бот, и в этот момент он
    /// точно знает, кто перед ним. Поле <c>kind</c> отличает код от токена.
    /// </summary>
    public string IssueLoginCode(long userId, string name, string? username)
    {
        var payload = new LoginCodePayload
        {
            Uid = userId,
            Name = name.Length > 0 ? name : "Без имени",
            Handle = string.IsNullOrEmpty(username) ? null : "@" + username,
            Exp = Now + LoginCodeTtlSeconds,
            Kind = "code",
        };
        return Sign(JsonSerializer.SerializeToUtf8Bytes(payload, Json));
    }

    public Viewer ReadLoginCode(string code)
    {
        var body = Verify(code, "код входа повреждён", "подпись кода не совпала");
        var payload = JsonSerializer.Deserialize<LoginCodePayload>(body, Json) ?? throw new AuthException("код входа повреждён");
        // Разделение видов обязательно: иначе долгий токен сессии сошёл бы за код входа.
        if (payload.Kind != "code")
        {
            throw new AuthException("это не код входа");
        }

        if (payload.Uid <= 0)
        {
            throw new AuthException("в коде нет пользователя");
        }

        if (payload.Exp < Now)
        {
            throw new AuthException("код входа истёк");
        }

        return new Viewer(payload.Uid, string.IsNullOrEmpty(payload.Name) ? "Без имени" : payload.Name, payload.Handle);
    }
    ...
    public Viewer VerifyInitData(string initData)
    {
        ...
        var secretKey = HMACSHA256.HashData(Encoding.UTF8.GetBytes("WebAppData"), _secret);
    ...
    private sealed class LoginCodePayload
    {
