...
{
    /// <summary>Токен бота: им подписаны данные запуска Mini App (initData) и им бот пишет в Telegram.</summary>
    public required string TelegramToken { get; init; }

    /// <summary>
    /// Секрет токенов сессии Mini App — отдельный от токена бота: утечка одного
    /// не должна отдавать и бота, и все сессии.
    /// </summary>
    ...
    /// <summary>Адрес Mini App для кнопки в сообщениях бота. Только https, иначе кнопка отключена.</summary>
    ...
    public static AppOptions Load(IConfiguration configuration, ILogger? logger = null)
    {
        ...
        {
            throw new InvalidOperationException("SESSION_SECRET не задан — им подписываются токены сессии Mini App");
        }
