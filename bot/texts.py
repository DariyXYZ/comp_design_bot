...
# Совет про /start повторяется в нескольких сообщениях об ошибке — и здесь, и
# в Mini App (`web/src/config/copy.ts`). Он идёт первым, потому что чаще всего
# и помогает: обновляет кнопку приложения, код входа внутри неё и меню.
RESTART_HINT = "Отправьте /start — бот обновит кнопку приложения и меню."

SESSION_RESET = (
    "Эта кнопка из старой сессии — после перезапуска бота черновики сбрасываются.\n"
    "Заявка оформляется в приложении: кнопка «✦ Возможности отдела» внизу.\n"
    + RESTART_HINT
)

NO_REQUESTS = (
    "Вы пока не отправляли заявок.\n"
    "Откройте приложение кнопкой «✦ Возможности отдела» — заявка создаётся оттуда: "
