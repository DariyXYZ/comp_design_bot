...
> **Статус**: рабочий прототип, собранный вне разработки (без тестов и CI). Передаётся «как есть» для переноса на боевую архитектуру — структура и стек ниже даны как отправная точка, а не как требование её сохранять.

В репозитории две реализации одной и той же архитектуры:

- **`bot/`** — оригинал (Python 3 / aiogram 3 / aiosqlite).
- **`bot-csharp/`** — построчный порт на C# (.NET 8 / Telegram.Bot / Microsoft.Data.Sqlite), та же FSM, та же схема БД, тот же формат карточки заявки. Детали переноса и чем механика отличается от aiogram (без изменения поведения) — в [bot-csharp/README.md](bot-csharp/README.md). Собран и проверен смоук-тестом слоя БД; живого прогона с реальным Telegram-токеном не было.

Обе версии читают один и тот же набор переменных `.env` и умеют работать с одним и тем же файлом `requests.sqlite3` — но **не запускайте обе одновременно на одном TELEGRAM_TOKEN** (Telegram отдаёт обновления только одному процессу-получателю, второй начнёт падать с `409 Conflict`).

## Структура файлов

```
bot/                      # весь код бота (aiogram 3, Python) — см. bot-csharp/README.md для C#-версии
...
bot-csharp/                # тот же бот на C# (.NET 8) — см. bot-csharp/README.md
